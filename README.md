# AgentDispatch Docs

Documentation repository for the `agent-dispatch` GitHub organization.

- `docs/technical-design.md`: provider-neutral OSS architecture.
- `docs/aws-agentcore-adapter.md`: AWS AgentCore adapter design.
- `docs/agentcore-quickstart.md`: end-to-end AWS AgentCore quickstart.
- `docs/package-consumption.md`: package install, release order, and MCP consumption guide.
- `docs/future-provider-adapters.md`: guide for adding more cloud providers without MCP changes.
- `docs/agent-core-runtime-design.md`: product/runtime design.
